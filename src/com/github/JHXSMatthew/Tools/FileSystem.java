...
public class FileSystem {
	...
	public FileSystem(String filename, String pluginname, String Folder) {
		...
		//logger.info(Folder);
		//logger.info(Bukkit.getPluginManager().getPlugin(pluginname).getDataFolder() + "\\" + Folder);
		//logger.info("fuck?");
	...
	public void addInt( String term , int input) {
		if(!existFile()) {
			create();
		}
		config.set( term , input);
		try {
			config.save(pFile);
		} catch (IOException e) {
		//	Log.Error(e.getMessage());
	...
	public void addBasicsString(String term , String input) {
		if(!existFile()) {
			create();
		}
		config.set(term , input);
	...
	public int getInt(String term) {
		if(!existFile()) {
			System.out.println("File not Found");
			return 0;
		}
		int returnValue = config.getInt(  term);
		return returnValue;
		
	}
	
	public double getBasicsdouble( String term) {
		if(!existFile()) {
			System.out.println("File not Found");
			return 0;
		}
		double returnValue = config.getDouble(term);
		return returnValue;
		
	}
	public boolean getBoolean(String filename) {
		if(!existFile()) {
			System.out.println("File not Found");
			return false;
		}
		boolean returnValue = config.getBoolean(filename);
